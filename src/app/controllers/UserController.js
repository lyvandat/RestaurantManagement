...
class UserController {
    ...
    cart(req, res, next) {
        Cart.findOne({ userID: req.params.id })
            .then((cart) => {
                const cartProducts = cart.products.map(item => mongoose.Types.ObjectId(item.productID));

                Product.find({ '_id': { $in: cartProducts } })
                    .then((products) => {
                        const items = multipleMongooseToObject(products);
                        items.forEach(item => Object.assign(item, {status: item.stock > 0 ? 'Còn hàng' : 'Hết hàng'}));
